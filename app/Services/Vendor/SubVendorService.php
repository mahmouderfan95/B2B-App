...
use App\Repositories\Vendor\SubVendorRepository;
use Exception;
...
class SubVendorService
{
    ...
    const VENDOR_SUB_VENDORS_INDEX = 'vendor.sub.list';
    ...
    public function SubVendorProfile(): \Illuminate\Contracts\View\View|\Illuminate\Http\JsonResponse
    {
        try {
            $subVendor = auth()->user();
            $roles = Role::where('guard_name','sub_vendor')->get();
            return view("sub-vendor.subVendors.edit", compact('subVendor','roles'));
        } catch (Exception $e) {
            return response()->json([
                'status' => 'general_error',
     ...
     * @param SubVendorRequest $request
     * @return RedirectResponse | array
     */

    public function updateSubVendor(SubVendorRequest $request, int $sub_vendor_id): RedirectResponse|array
    {
        $req = $request->all();
        if (isset($req['password']))
            $req['password'] = bcrypt($req['password']);
        ...
        try {
            ...
            if ($vendor) {
                if (auth()->guard('sub_vendor')->check()) {
