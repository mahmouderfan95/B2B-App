...
class SpecialOrderService
{
    ...
    public function acceptedByVendor($id)
    {
        try{
            // get sp order by id
            $spOrder = $this->specialOrderRepository->getSpOrderById($id);
            // get order quotation by id
            $orderQuotation = OrderQuotation::where('special_order_id',$id)->first();
            // update sp order status
            $spOrder->update(['status' => OrderStatus::ACCEPTED]);
            // update order quotations
            $orderQuotation ? $orderQuotation->update(['status' => 'accepted']) : '';
            // update order quotation histories
            isset($orderQuotation->histories) ? OrderQuotationHistories::where('order_quotation_id',$orderQuotation->id)->where('special_order_id',$spOrder->id)->update([
                'status' => 'accepted'
            ]) : '';
    ...
    public function rejectByVendor($id)
    {
        try{
            ...
            $orderQuotation = OrderQuotation::where('special_order_id',$id)->first();
            // update sp order status
            $spOrder->update(['status' => OrderStatus::CANCELED]);
            // update order quotations
            $orderQuotation ? $orderQuotation->update(['status' => 'refused']) : '';
            // update order quotation histories
            isset($orderQuotation->histories) ? OrderQuotationHistories::where('order_quotation_id',$orderQuotation->id)->where('special_order_id',$spOrder->id)->update([
                'status' => 'refused'
            ]) : '';
    ...
    {
        $order = SpecialOrder::where('id',$id)->where('status',OrderStatus::PAID30)->first();
        if($order)
        {
    ...
    {
        $order = SpecialOrder::where('id',$id)->where('status',OrderStatus::PAID)->first();
        if($order)
        {
