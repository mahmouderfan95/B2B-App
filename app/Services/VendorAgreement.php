...
class VendorAgreement {
    ...
    public function fillPdfFile(
        ...
        $font = public_path("build/fonts/cairoreg.ttf");
        $boldFont = public_path("build/fonts/cairobold.ttf");

        $vendorSign = $Arabic->utf8Glyphs(__('admin.vendors-agreements-keys.vendor-sign'));
        $text = $Arabic->utf8Glyphs($vendorUser->name);

        $pdfPageWidth = 793.7008;
        $now = $approvedDateTime ? $approvedDateTime : now()->toDateTimeString();
        $dateString = $Arabic->utf8Glyphs(__('admin.vendors-agreements-keys.approve-date'));
        $columnString = " : ";
        ...
        imagettftext(
            ...
            self::textFontSize,
            0,
            ($pdfPageWidth + self::rightPadding) - (self::pagePadding + (strlen($vendorSign) * self::textWidth) + (strlen($text) * self::textWidth) - self::spaceWidth + 10),
            self::textYaxis,
            $black,
            $font,
            $text
