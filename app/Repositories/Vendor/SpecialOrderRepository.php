...
class SpecialOrderRepository extends BaseRepository
{
    ...
    public function getSpOrderById($id)
    {
        return $this->model->where('id',$id)->first();
    }
    public function getAllSpecialOrders(): \Illuminate\Database\Eloquent\Collection
    {
        return $this->model->where('vendor_id', auth('vendor')->id())->with(['client', 'products'])
            ->get();
    }

    public function show($id)
    {
        return $this->model->where('id', $id)->where('vendor_id', auth('vendor')->id())->with(['vendor', 'client', 'products', 'shipping_special_offers', 'order_quotations'])->first();
    }
    ...
    public function accept($data_request)
    {
        $special_order = $this->model->find($data_request['special_order_id']);
        $special_order->total = $data_request['total'];
        $special_order->status = OrderStatus::ACCEPTED;
        $special_order->save();
