        ...
        <div class="col-xl-9">
            <div class="card">
                ...
                <div class="card-body">
                            ...
                            <tr class="border-top border-top-dashed">
                                ...
                                <td colspan="2" class="fw-medium p-0">
                                    <table class="table table-borderless mb-0">
                                        <tbody>
                                        <tr class="border-top border-top-dashed">
                                            <th scope="row">@lang('admin.orders.total_expect') :</th>
                                            <td class="text-end">@if($specialOrder->total != NULL)
                                                    {{$specialOrder->total}}
                                                @else
                                                        ...
                                                        <div class="flex-shrink-0 mt-2 mt-sm-0">
                                                            <a href="javasccript:void(0;)"
                                                               ...
                                                               data-bs-target="#myModalOut">
                                                                <i class="ri-add-circle-line align-middle me-1"></i> @lang('admin.orders.add_total_expect')
                                                            </a>
                                                        </div>
                                                    </div>
                                                @endif</td>
                    ...
                    <div id="myModalOut" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel"
                         aria-hidden="true" style="display: none;">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title"
                                        id="myModalLabel">@lang('admin.orders.add_total_expect')</h5>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <form class="row g-3 needs-validation" novalidate
                                                  action="{{ route('vendor.quotations.add_total') }}" method="post">
                                                @csrf
                                                <input type="hidden" name="special_order_id"
                                                       value="{{ $specialOrder->id }}">

                                                <input type="number" name="total" class="form-control mb-2" required>
                                                <button class="btn btn-success"
                                                        type="submit"> @lang('admin.btn.add') </button>
                                            </form>
                                        </div>
                                    </div>
                    ...
                    <div class="d-sm-flex align-items-center">
                        ...
                        <div class="flex-shrink-0 mt-2 mt-sm-0">
                            <a href="javasccript:void(0;)" class="btn btn-soft-primary btn-sm mt-2 mt-sm-0"><i
                                    class="ri-map-pin-line align-middle me-1"></i> {{ \App\Enums\OrderStatus::getStatusWithClass($specialOrder->status)["name"] }}
                            </a>
                        </div>
                    </div>
                </div>
                    ...
                    <table id="buttons-datatables" class="display table table-boffered" style="width:100%">
                        ...
                        @forelse($specialOrder->order_quotations as  $order_quotation)
                            <tr>
                                ...
                                <td> @if($order_quotation->status == 'accepted')
                                        <span class="badge badge-soft-success">@lang('admin.accepted')</span>
                                    @elseif($order_quotation->status == 'pending')
                                        <span class="badge badge-soft-primary">@lang('admin.pending')</span>
                                    @else
                                        <span class="badge badge-soft-danger">@lang('admin.refused')</span>
                                    @endif
                                </td>
                                <td> @if($order_quotation->sender_type == 'vendor')
                                ...
                                <td> @if($order_quotation->sender_type == 'client' && $order_quotation->status != 'accepted')
                                        <form action="{{route('vendor.quotations.accept')}}"
                                              method="post" style="width: 50px;display: inline-block;">
                                            @csrf
                                            <input type="hidden" name="special_order_id"
                                                   value="{{ $specialOrder->id }}">
                                            <input type="hidden" name="quotation_id" value=" {{$order_quotation->id}}">
                                            <input type="hidden" name="total"
                                                   value="{{$order_quotation->quotation_price}}"
                                                   class="form-control mb-2" required>
                                            <button type="submit" class="btn btn-success">
                                                <i class="bx bx-check"></i>
                                            </button>
                                        </form>
                                        <form action="{{route('vendor.quotations.refused')}}"
                                              method="post" style="width: 50px;display: inline-block;">
                                            @csrf
                                            <input type="hidden" name="special_order_id"
                                                   value="{{ $specialOrder->id }}">
                            ...
                            <tr>
                                ...
                                <td>
                                     @if( $offer->status == 'pending')
                                        <form action="{{route('vendor.shipping.special.offer.accept')}}"
                                              method="post" style="width: 50px;display: inline-block;">
                                            @csrf
                                            <input type="hidden" name="special_order_id"
                                                   value="{{ $specialOrder->id }}">
                                            <input type="hidden" name="special_offer_id" value=" {{$offer->id}}">
                                            <input type="hidden" name="total"
                                                   value="{{$offer->price}}"
                                                   class="form-control mb-2" required>
                                            <button type="submit" class="btn btn-success">
                                        ...
                                        <form action="{{route('vendor.shipping.special.offer.refused')}}"
                                              method="post" style="width: 50px;display: inline-block;">
                                            @csrf
                                            <input type="hidden" name="special_order_id"
                                                   value="{{ $specialOrder->id }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <ul class="list-unstyled mb-0 vstack gap-3">
                        ...
                        <li>
                            <i class="ri-mail-line me-2 align-middle text-muted fs-16"></i> {{$specialOrder->client->email}}
                        </li>
                        <li>
                            <i class="ri-phone-line me-2 align-middle text-muted fs-16"></i>{{$specialOrder->client->phone}}
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0"><i
                            class="ri-map-pin-line align-middle me-1 text-muted"></i> @lang('admin.orders.shipping_details')
                    </h5>
                </div>
                <div class="card-body">
